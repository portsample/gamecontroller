...
public class MainActivity extends AppCompatActivity implements InputManager.InputDeviceListener {
    ...
    private void updateGameControllerCount() {
        ...
        if (gameControllerIds.size() == 0) {
            mTextView.setText("Not connected to Bluetooth controller!");
        } else {
            mTextView.setText(String.format(Locale.getDefault(), "Connected to %d Bluetooth controller!", gameControllerIds.size()));
        }
    }
    ...
    static public int getControllerFlagsForDevice(InputDevice inputDevice, Context context) {
        int iControllerFlags = 0;
        BatteryManager batteryManager = (BatteryManager) context.getSystemService(Context.BATTERY_SERVICE);
        if (batteryManager != null) {
            int batteryLevel = batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY);
            if (batteryLevel >= 0) {
                iControllerFlags |= DEVICEFLAG_BATTERY;
            }
        }
        return iControllerFlags;
    }

    public void AndroidBatteryLevel(InputDevice inputDevice) {
        BatteryManager batteryManager = (BatteryManager) getSystemService(Context.BATTERY_SERVICE);
        if (batteryManager != null) {
            int batteryLevel = batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY);
            Toast.makeText(this, "Android Battery Level: " + batteryLevel + "%", Toast.LENGTH_SHORT).show();
        }
    }
    public void GamePadBatteryLevel(InputDevice inputDevice) {
        int batteryLevel = 0;
     //   InputDevice.SOURCE_GAMEPAD inputDevice;
     //   batteryLevel = inputDevice.getBatteryState();
        Toast.makeText(this, "Gamepad Battery Level: " + batteryLevel + "%", Toast.LENGTH_SHORT).show();
        //}
    }
        ...
        if ((event.getSource() & InputDevice.SOURCE_GAMEPAD) == InputDevice.SOURCE_GAMEPAD) {
            ...
            switch (event.getKeyCode()) {
                case KeyEvent.KEYCODE_BUTTON_X:
                    keyName = "BUTTON_X";
                    AndroidBatteryLevel(InputDevice.getDevice(event.getDeviceId()));
                    break;
                case KeyEvent.KEYCODE_BUTTON_Y:
                    keyName = "BUTTON_Y";
                    GamePadBatteryLevel(InputDevice.getDevice(event.getDeviceId()));
                    break;
                case KeyEvent.KEYCODE_BUTTON_A:
                    keyName = "BUTTON_A";
                    Toast.makeText(this, "Button A toast event.", Toast.LENGTH_SHORT).show();
                    break;
